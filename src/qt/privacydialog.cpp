...
#include "zmagocoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMAGO ought to be enough for anybody." - Bill Gates, 2017
    ui->zMAGOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMAGOSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMAGO->setEnabled(false);
        ui->pushButtonMintzMAGO->setToolTip(tr("zMAGO is currently disabled due to maintenance."));

        ui->pushButtonSpendzMAGO->setEnabled(false);
        ui->pushButtonSpendzMAGO->setToolTip(tr("zMAGO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMAGOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMAGO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMAGO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMAGO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMAGO in ") +
...
void PrivacyDialog::on_pushButtonSpendzMAGO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMAGO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMAGO
        sendzMAGO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMAGO
    sendzMAGO();
}

void PrivacyDialog::on_pushButtonZMagocoinControl_clicked()
{
    ZMagocoinControlDialog* zMAGOControl = new ZMagocoinControlDialog(this);
    zMAGOControl->setModel(walletModel);
    zMAGOControl->exec();
}

void PrivacyDialog::setZMagocoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMAGOSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMAGO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Magocoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMAGOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMAGO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MAGOIPCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMAGO</b>";
    ...
    // use mints from zMAGO selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMagocoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMagocoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zmagocoin selector in case it was used
    ZMagocoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMAGO Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Magocoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMAGO Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMAGO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMAGO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMAGO "));
    ui->labelzMAGOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMAGOSyncStatus->setVisible(fShow);
}
