...
#include "ui_zmagocoincontroldialog.h"
...
std::list<std::string> ZMagocoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMagocoinControlDialog::listMints;

ZMagocoinControlDialog::ZMagocoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMagocoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMagocoinControlDialog::~ZMagocoinControlDialog()
{
    delete ui;
}

void ZMagocoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMagocoinControlDialog::updateList()
{
...
void ZMagocoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMagocoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMagocoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMagocoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMagocoinControlDialog::GetSelectedMints()
{
...
void ZMagocoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
