...
VERSION=
commit=false
url=https://github.com/magocoin/magocoin
proc=2
mem=2000
lxc=true
...
-u|--url	Specify the URL of the repository. Default is https://github.com/magocoinproject/magocoin
...
then
    ...
    git clone https://github.com/magocoin/gitian.sigs.git
    git clone https://github.com/magocoin/magocoin-detached-sigs.git
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit magocoin=${COMMIT} --url magocoin=${url} ../magocoin/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign -p $signProg --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../magocoin/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/magocoin-*.tar.gz build/out/src/magocoin-*.tar.gz ../magocoin-binaries/${VERSION}
	popd
